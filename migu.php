    ...
    private function dumpM3u()
    {
        ...
        foreach ($this->channelList as $group => $groupList) {
            foreach ($groupList as $channelId => $channelName) {
                $str .= sprintf('#EXTINF:-1 group-title="%s" tvg-id="%s" tvg-name="%s" tvg-logo="https://live.fanmingming.com/tv/%s.png",%s%s%s%s%s', $group, $channelName, $channelName, $channelName, $channelName, PHP_EOL, $this->selfJumpUrl, $channelId, PHP_EOL);
            }
        }
        return $str;
    }
